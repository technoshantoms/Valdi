// ---- Generator helper
function __tsn_async_helper<T>(
  gen: Iterator<Promise<any>, T, any>,
): Promise<T> {
...
function __tsn_async_generator_helper(
  gen: Iterator<AsyncGeneratorStep, any, any>,
  ...
  return {
    ...
    async next(a: any): Promise<IteratorResult<any, any>> {
      try {
        ...
        return Promise.resolve(res.done ? res.value : res.value.yield).then(
          (v) => ({ value: v, done: res.done }),
        );
      } catch (e: any) {
...
function __tsn_get_iterator<T>(
  gen: Iterable<T> | AsyncIterable<T>,
...
function __tsn_get_async_iterator<T>(
  gen: Iterable<T> | AsyncIterable<T>,
): Iterator<T> | AsyncIterator<T> {
...
// inject the helper functions to global namespace for the compiler
