...
static void add_header(mapping** hashmap, char* header, char* import_path);
...
static void parse_args(mapping** entries, char** av, int ac) {
    ...
    for (; *av; av += 2) {
        ...
        if (!*from) {
            fprintf(stderr, "Mapping pair missing element");
            exit(1);
        }
        ...
        if (**from == '@') {
            ...
            return; // there should be no more args after a param file
        }

        char** to = av + 1;
        add_header(entries, *from, *to);
    }
}

static void add_header(mapping** hashmap, char* header, char* import_path) {
    char* key = strdup(import_path);
    if (!key) {
        perror("malloc");
        exit(1);
    }
