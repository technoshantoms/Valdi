...
public:
    ...
    static CppType toCpp(JNIEnv* /*jniEnv*/, JniType j) noexcept {
        return static_cast<CppType>(j);
    }
    static JniType fromCpp(JNIEnv* /*jniEnv*/, CppType c) noexcept {
        return static_cast<JniType>(c);
    }
    ...
    static CppType toCpp(JNIEnv* jniEnv, JniType j) {
...

        {
            ...
            std::unique_ptr<uint8_t, decltype(deleter)> ptr(
                reinterpret_cast<uint8_t*>(jniEnv->GetPrimitiveArrayCritical(j, nullptr)), deleter);
...
struct MapJniInfo {
    ...
    const jmethodID constructor{jniGetMethodID(clazz.get(), "<init>", "()V")};
    const jmethodID method_put{
        jniGetMethodID(clazz.get(), "put", "(Ljava/lang/Object;Ljava/lang/Object;)Ljava/lang/Object;")};
    const jmethodID method_size{jniGetMethodID(clazz.get(), "size", "()I")};
...
public:
    ...
    static CppType toCpp(JNIEnv* jniEnv, JniType j) {
        ...
        auto deleter = [jniEnv, &bytes](void* c) {
            if (c) {
                jniEnv->ReleasePrimitiveArrayCritical(bytes.get(), c, JNI_ABORT);
            }
        };
        std::unique_ptr<uint8_t, decltype(deleter)> ptr(
            reinterpret_cast<uint8_t*>(jniEnv->GetPrimitiveArrayCritical(bytes.get(), nullptr)), deleter);
        if (!ptr) {
            jniExceptionCheck(jniEnv);
        }
    ...
    static LocalRef<JniType> fromCpp(JNIEnv* jniEnv, const CppType& c) {
        ...
        if (cppBuf.size() > 0) {
            ...
            javaBuf = LocalRef<jobject>(
                jniEnv, jniEnv->CallStaticObjectMethod(byteBuffer.clazz.get(), byteBuffer.method_allocate, 0));
            jniExceptionCheck(jniEnv);
        }
...
public:
    ...
    static LocalRef<JniType> fromCpp(JNIEnv* jniEnv, const CppType& c) {
        auto j = LocalRef<JniType>(
            jniEnv, jniEnv->NewObjectArray(static_cast<jsize>(c.size()), JniClass<Array>::get().clazz.get(), nullptr));
...
public:
    ...
    static CppType toCpp(JNIEnv* jniEnv, JniType j) {
        ...
        auto deleter = [jniEnv, arr](void* c) {
            if (c) {
                jniEnv->ReleasePrimitiveArrayCritical(arr, c, JNI_ABORT);
            }
        };
        std::unique_ptr<EJniType, decltype(deleter)> ptr(
            reinterpret_cast<EJniType*>(jniEnv->GetPrimitiveArrayCritical(arr, nullptr)), deleter);
        if (!ptr) {
            jniExceptionCheck(jniEnv);
            return {};
        }
    ...
    static LocalRef<JniType> fromCpp(JNIEnv* jniEnv, const CppType& c) {
        ...
        if (c.size() > 0) {
            auto deleter = [jniEnv, j](void* c) {
                if (c) {
                    jniEnv->ReleasePrimitiveArrayCritical(j, c, 0);
                }
            };
            std::unique_ptr<EJniType, decltype(deleter)> ptr(
                reinterpret_cast<EJniType*>(jniEnv->GetPrimitiveArrayCritical(j, nullptr)), deleter);
