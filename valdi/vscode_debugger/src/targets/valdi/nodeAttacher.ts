...
import { AnyLaunchConfiguration, INodeAttachConfiguration, IValdiAttachConfiguration } from '../../configuration';
...
import { NodeBinaryProvider, NodeBinary } from '../node/nodeBinaryProvider';
...
export class NodeAttacher extends NodeAttacherBase<INodeAttachConfiguration> {
  ...
  protected resolveParams(params: AnyLaunchConfiguration): INodeAttachConfiguration | undefined {
    return params.type === DebugType.SnapValdi && params.request === 'attach' ? params as any : undefined;
  }
  ...
  protected async launchProgram(runData: IRunData<INodeAttachConfiguration>): Promise<void> {
    const doLaunch = async (
      ...
      const watchdog = await WatchDog.attach({
        ...
        dynamicAttach: true,
        dieWithTarget: (runData.params as unknown as IValdiAttachConfiguration).dieWithTarget
      });
