...
import { IDefaultBrowserProvider } from '../../common/defaultBrowserProvider';
...
export interface IWatchdogInfo {
  ...
  /**
   * If set to true, kill the watchdog when the target dies.
   */
...
export class WatchDog implements IDisposable {
  ...
  public static async attach(info: IWatchdogInfo) {
    ...
    return new WatchDog(info, server, info.dieWithTarget);
  }

  constructor(private readonly info: IWatchdogInfo, private readonly server: ITransport, private readonly dieWithTarget: boolean = false) {
    this.listenToServer();
  }
