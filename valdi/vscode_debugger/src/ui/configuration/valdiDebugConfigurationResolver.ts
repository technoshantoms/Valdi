...
import {
  ...
  ResolvingValdiLaunchConfiguration,
  AnyValdiConfiguration,
...
type ResolvingValdiConfiguration =
  | ResolvingValdiAttachConfiguration
  | ResolvingValdiLaunchConfiguration;
 ...
 */
@injectable()
export class ValdiConfigurationResolver extends BaseConfigurationResolver<AnyValdiConfiguration> {
  constructor(
  ...
  public async resolveDebugConfigurationWithSubstitutedVariables(
    ...
    const config = rawConfig as AnyValdiConfiguration;
    if (
      config.type === DebugType.SnapValdi &&
      config.request === 'attach' &&
      typeof config.processId === 'string'
    ) {
      await resolveProcessId(config as any);
    }
  ...
  protected async resolveDebugConfigurationAsync(
  ...
  ): Promise<AnyValdiConfiguration | undefined> {
    ...
    if (config.request === 'launch') {
      ...
      fixInspectFlags(config as any);

      // update outfiles to the nearest package root
      await guessOutFiles(folder, config);
    }

    return applyValdiDefaults(config);
  }

  protected getType() {
    return DebugType.SnapValdi as const;
  }

  /**
   * @override
   */
  protected getSuggestedWorkspaceFolders(config: AnyValdiConfiguration) {
...
async function guessOutFiles(
  folder: vscode.WorkspaceFolder | undefined,
  config: ResolvingValdiLaunchConfiguration,
) {
...
export function createLaunchConfigFromContext(
  folder: vscode.WorkspaceFolder | undefined,
  resolve: boolean,
  existingConfig?: ResolvingValdiConfiguration,
): ResolvingValdiConfiguration {
  const config: ResolvingValdiConfiguration = {
...
function configureMern(config: ResolvingValdiConfiguration) {
  if (config.request !== 'launch') {
    return;
  }
